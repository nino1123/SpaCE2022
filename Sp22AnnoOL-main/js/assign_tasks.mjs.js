...
class TaskAssigner {
  constructor(core, lo) {
    ...
    this.tasks_idx_base = core.tasks_idx_base ?? this.tasks.length;
    this.lo = lo;  //: Lodash
  }
  ...
  static build(pack, lo) {
    ...
    let tasks_idx_base = pack.tasks_idx_base ?? tasks.length;
  ...
  static _get_polygraphs(entries, polygraphs_per_user) {
    ...
    for (let entry of entries) {
      if ('polygraph' in entry) {
        let polygraph_type = entry['polygraph'];
        if (!(entry['polygraph'] in polygraphs)) {
          throw new Error(`!(entry['polygraph'] in polygraphs)`);
        };
  ...
  _create_candidate_tasks(candidate_entries) {
    ...
    let task_index = this.tasks_idx_base;
  ...
  _assign_polygraph_tasks_to_users(polygraph_tasks) {
    for (let user of this.users) {
      ...
      for (let polygraph_type of Object.keys(polygraph_tasks)) {
        ...
        let tasks = this.lo.sampleSize(polygraphs, polygraphs_num)
        for (let task of tasks) {
