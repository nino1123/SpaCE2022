...
class Examiner:
    ...
    def examine_single_user(self, user: Dict):
        ...
        '''
        polygraph_tasks = self._get_polygraph_tasks_of_single_user(user)
        assert len(polygraph_tasks) == len(polygraph_annos), \
                f'User {user["id"]}: Numbers of polygraph tasks and annotations are not match'
        '''
...
if __name__ == '__main__':
    ...
    results = examine(entries, tasks, users, annos, 'task0-02')
